...
## 15. Document Metadata — student handles this herself now
**Scope: universal for every `.docx` file built and submitted from this repo.**

- **Do not edit `.docx` metadata (docProps/core.xml, docProps/app.xml, or scan/patch for
  generator fingerprints) unless the student explicitly asks for it in that specific
  request.** The student has said she will handle metadata cleanup (creator name, app.xml
  word/paragraph statistics, removing `python-docx` fingerprints, etc.) herself going
  forward, specifically because the extra read/patch/rebuild/re-verify cycle this used to
  require was consuming a large amount of session effort for a step she is fine doing on her
  own. Generate the `.docx` content itself (text, table, formatting, real hyperlinks per
  Section 1) and stop there — do not automatically follow up with a metadata patching pass
  "for completeness."
- If the student does ask for a metadata fix in a given request, it's fine to do it then,
  using the same field list this section used to document (creator/lastModifiedBy = her
  real name from `student-profile-*.md`; plausible non-zero `app.xml` stats; no
  `python-docx`/`python_docx` fingerprints anywhere in the zip) — just don't do it by
  default anymore.
